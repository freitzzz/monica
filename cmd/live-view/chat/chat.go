...
func Main2() {
	// Run the server.
	http.Handle("/", live.NewHttpHandler(live.NewCookieStore("session-name", []byte("weak-secret")), NewHandler()))
...
func NewHandler() live.Handler {
	t, err := template.ParseFiles("cmd/live-view/chat/layout.html", "cmd/live-view/chat/view.html")
	if err != nil {
		log.Fatal(err)
	}
